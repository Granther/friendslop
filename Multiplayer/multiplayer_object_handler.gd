		...
		if frame_count == N_FRAMES_PER_UPDATE: # Time to rsync
			for obj in tracked_objects:
				...
				obj.get_multi_comp().resync_to_clients.rpc(
					obj.global_position, obj.global_rotation, obj.linear_velocity, #obj.get_path()
				)
			frame_count = 0
		else: 
			frame_count += 1
...
#var _frame_count = 0
#var tracked_objects: Array[RigidBody3D]
#var _on_update_frame: bool = false
#
## We are going to assume that all nodes that use this will have the get_multi_comp method
#func register_object(obj: Node3D): 
	#tracked_objects.append(obj)
...
#func _physics_process(delta: float) -> void:
	#if _frame_count == N_FRAMES_PER_UPDATE: # Time to rsync
		#_on_update_frame = true
		#_frame_count = 0
	#else: 
		#_on_update_frame = false
		#_frame_count += 1
#
#func is_update_tick() -> bool:
	#return _on_update_frame
